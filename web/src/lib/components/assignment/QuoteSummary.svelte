...
<div class="flex h-full flex-col overflow-y-auto">
	...
	{#if loading}
		<div class="space-y-2 px-3 py-2">
			...
			<div class="h-4 animate-pulse rounded bg-gray-50 w-2/3"></div>
			<div class="h-4 animate-pulse rounded bg-gray-50 w-1/2"></div>
		</div>
	{:else if !quote || quote.line_items.length === 0}
		<div class="px-3 py-6 text-center">
			<p class="text-2xl font-bold text-gray-300">$0.00</p>
			<p class="mt-1 text-xs text-gray-400">Assign materials to see quote</p>
		</div>
	{:else}
		<!-- Total -->
		<div class="border-b border-gray-200 px-3 py-3">
			<p class="text-2xl font-bold text-gray-900">{fmt(quote.total)}</p>
			{#if quote.tax}
		...
		<!-- Line items -->
		<div class="px-3 py-2">
			<h4 class="text-[11px] font-medium uppercase text-gray-400 mb-1">Line Items</h4>
			{#each quote.line_items as li (li.zone_id + li.material_id)}
				<div class="border-t border-gray-50 py-1.5">
					<div class="flex items-center justify-between">
						<span class="text-xs text-gray-700 truncate max-w-[60%]">
							{li.zone_label ?? 'Zone'}
						</span>
						<span class="text-xs font-medium text-gray-900 font-mono">{fmt(li.line_total)}</span>
					</div>
